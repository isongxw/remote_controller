...
def handle_mouse():
    ...
    try:
        ...
        if action == 'move':
            ...
            return jsonify({'status': 'success', 'message': f'滚轮滚动 dx={dx}, dy={dy}'})
            ...
            return jsonify({'status': 'success', 'message': f'拖拽操作完成: ({start_x},{start_y}) -> ({end_x},{end_y})'})
        
        elif action == 'position':
            pos = mouse_service.get_position()
            return jsonify({'status': 'success', 'position': {'x': pos[0], 'y': pos[1]}})
