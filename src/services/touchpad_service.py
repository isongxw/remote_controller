...
class TouchpadService:
    ...
    def detect_touch_mode(self, touches_data):
        ...
        print(f"[DEBUG] 检测触摸模式: touch_count={touch_count}")
    ...
    def handle_touch_start(self, touches_data):
        ...
        Returns:
            dict: 响应数据
        """
        print(f"[DEBUG] 收到触摸开始数据: {touches_data}")  # 添加调试日志
        ...
        print(f"[DEBUG] 触摸开始记录: touch_id={touch_id}, 位置=({x}, {y})")
    ...
    def handle_touch_move(self, touches_data):
        ...
        Returns:
            dict: 响应数据
        """
        print(f"[DEBUG] 收到触摸移动数据: {touches_data}")  # 添加调试日志
        
        touch_id = touches_data.get('touch_id', 'default')
        touches = touches_data.get('touches', [])
        
        if not touches or touch_id not in self.touchpad_state['active_touches']:
            print(f"[DEBUG] 无效的触摸移动: touch_id={touch_id}, touches={len(touches)}, active_touches={list(self.touchpad_state['active_touches'].keys())}")
        ...
        dx = x - prev_x
        dy = y - prev_y
        total_distance = ((x - touch_state['start_x'])**2 + 
                         (y - touch_state['start_y'])**2)**0.5
        
        print(f"[DEBUG] 移动计算: dx={dx}, dy={dy}, total_distance={total_distance}")
        ...
        if mode == 'single':
            if total_distance > self.config['MOVE_THRESHOLD']:
                ...
                if not self.touchpad_state['is_dragging']:
                    ...
                    print(f"[DEBUG] 移动鼠标: 从 {current_pos} 到 ({new_x}, {new_y})")
            ...
            print(f"[DEBUG] 滚动: scroll_dx={scroll_dx}, scroll_dy={scroll_dy}")
    ...
    def handle_touch_end(self, touches_data):
        ...
        Returns:
            dict: 响应数据
        """
        print(f"[DEBUG] 收到触摸结束数据: {touches_data}")  # 添加调试日志
        
        touch_id = touches_data.get('touch_id', 'default')
        
        if touch_id not in self.touchpad_state['active_touches']:
            print(f"[DEBUG] 无效的触摸结束: touch_id={touch_id}")
        ...
        # 检测触摸模式
        mode = self.detect_touch_mode(touches_data)
        print(f"[DEBUG] 触摸结束模式: {mode}")
        
        # 计算触摸持续时间和移动距离
        touch_duration = time.time() - touch_state['start_time']
        total_distance = ((touch_state['current_x'] - touch_state['start_x'])**2 + 
                         (touch_state['current_y'] - touch_state['start_y'])**2)**0.5
        
        print(f"[DEBUG] 触摸持续时间: {touch_duration:.3f}s, 移动距离: {total_distance:.1f}px")
        
        action_performed = None
        
        # 处理双指右键点击
        if mode == 'scroll' and touch_duration < self.config['DOUBLE_CLICK_TIME'] and total_distance < self.config['MOVE_THRESHOLD']:
            print("[DEBUG] 执行双指右键点击")
